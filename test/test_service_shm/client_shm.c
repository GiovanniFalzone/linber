...
void *thread_job(void *args){
	...
	if(abort_request == 0){
		...
		if(worker.blocking == 1){
			ret = linber_request_service_shm(worker.service_uri, worker.uri_len, REL_DEADLINE, service_request, service_request_len, &service_response, &service_response_len);
