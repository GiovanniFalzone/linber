...
int main(int argc,char* argv[]){
	...
	while(1){
		...
		printf("---------------------\n");
		printf("Services: %d, Concurrent Workers: %d, Requests: %d Serving: %d\n", num_services, num_concurent_workers, num_requests, serving_count);
		for(int i = 0; i< num_services; i++){
			service_status service = system.services[i];
			printf("Service %s, id: %lu, Concurrent Workers %d Requests:%d, Serving: %d, Serving time:%d\n", service.uri, service.id, service.max_concurrent_workers, service.requests_count, service.serving_requests_count, service.serving_time);
		}
	}
	linber_exit();
}
